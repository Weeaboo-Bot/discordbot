...
const util = require("util"), fs = require("fs"), path = require('path'),
      readdir = util.promisify(fs.readdir), mongoose = require("mongoose"),
...
const dbConnect = () => {
  ...
  mongoose
      ...
      .then(() => { console.log("Connected to the Mongodb database.", "log"); })
      .catch((err) => {
...
const init = async (res) => {
  ...
  custom_log.log(`Loading a total of ${directories.length} categories.`,
                 'success');
  for (const dir of directories) {
    const commands = await readdir("./commands/" + dir + "/");
    custom_log.log(`Loading a total of ${commands.length} commands in ${dir}`,
                   'success');
