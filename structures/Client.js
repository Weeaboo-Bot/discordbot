...
module.exports = class WeabooClient extends CommandoClient {
  constructor(config, options = {}) {
    ...
    this.on('commandError', (command, err) => this.logger.error(
    ...
    this.webhook = new Discord.WebhookClient(
        config.discord.DISCORD_WEBHOOK_ID, config.discord.DISCORD_WEBHOOK_TOKEN,
  ...
  loadEvents(path) {
    readdir(path, (err, files) => {
      ...
      if (files.length === 0)
        return this.logger.warn('No events found');
      ...
      files.forEach((f) => {
        ...
        delete require.cache[require.resolve(
            resolve(__basedir, join(path, f)))]; // Clear cache
  ...
  loadGroups() {
    ...
    this.registry.groups.forEach(
        (group) => { this.logger.info(`Loading group: ${group.name}`); });
    return this;
  }
  ...
  loadCommands() {
    ...
    this.registry.commands.forEach(
        (command) => { this.logger.info(`Loading command: ${command.name}`); });
    return this;
  }
  ...
  sendSystemErrorMessage(guild, error, errorMessage) {
    ...
    if (!systemChannel || !systemChannel.viewable ||
        !systemChannel.permissionsFor(guild.me).has(
