...
module.exports = class Battler {
  ...
  async chooseAction(msg) {
    if (this.bot) {
      ...
      if (this.canSpecial)
        botChoices.push('special');
      if (this.canHeal && this.hp < 200)
        botChoices.push('cure');
      return botChoices[Math.floor(Math.random() * botChoices.length)];
    }
    let content = stripIndents`
			${this}, do you ${
        list(choices.map((choice) => `**${choice}**`),
             'or')}? You have **${this.mp}** MP.
			**${this.battle.user.user.tag}:** ${
        this.battle.user.hp} HP
			**${this.battle.opponent.user.tag}:** ${
        this.battle.opponent.hp} HP
		`;
    ...
    const filter = (res) => {
      ...
      if (res.author.id === this.user.id && choices.includes(choice)) {
        if ((choice === 'special' && !this.canSpecial) ||
  ...
  get canHeal() { return this.mp > 0; }

  get canSpecial() { return this.mp >= 25; }

  get canFinal() { return this.hp < 100 && this.mp >= 50 && !this.usedFinal; }
