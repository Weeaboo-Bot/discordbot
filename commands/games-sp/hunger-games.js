...
module.exports = class HungerGamesCommand extends Command {
    ...
    try {
      ...
      while (remaining.size > 1) {
        if (!bloodbath && sun)
          ++turn;
        const sunEvents =
            bloodbath ? events.bloodbath : sun ? events.day : events.night;
        const results = [];
        const deaths = [];
        ...
        if (deaths.length) {
          ...
          text += stripIndents`
              ...
              deaths.length === 1 ? '' : 's'} can be heard in the distance.**
						${deaths.join('\n')}
					`;
        }
        text += '\n\n_Proceed?_';
  ...
  makeEvents(tributes, kills, eventsArr, deaths, results) {
    ...
    for (const tribute of tributes) {
      if (!turn.has(tribute))
        continue;
      const valid = eventsArr.filter((event) => event.tributes <= turn.size &&
                                                event.deaths < turn.size);
  ...
  makeLeaderboard(tributes, kills) {
    ...
    return tributes.filter((tribute) => kills[tribute] > 0)
