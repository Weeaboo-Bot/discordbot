...
module.exports = class HearingTestCommand extends Command {
  ...
  async run(msg) {
    ...
    if (!connection) {
      ...
      return msg.reply(
          `I am not in a voice channel. Use ${usage} to fix that!`);
    }
    try {
      let age;
      let range;
      ...
      for (const {age : dataAge, khz, file} of data) {
        ...
        await delay(3500);
        await msg.reply(
            'Did you hear that sound? Reply with **[y]es** or **[n]o**.');
      ...
      if (age === 'max') {
        return msg.reply(stripIndents`
					...
					The maximum frequency you were able to hear was **${
            range}000hz**.
				`);
      }
      return msg.reply(stripIndents`
				You have the hearing of someone **${
          Number.parseInt(age, 10) + 1} or older**.
				The maximum frequency you were able to hear was **${
          range}000hz**.
			`);
    } catch (err) {
