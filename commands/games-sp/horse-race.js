...
registerFont(
    path.join(__dirname, '..', '..', 'assets', 'fonts', 'Paladins.otf'),
    {family : 'Paladins'});
...
module.exports = class HorseRaceCommand extends Command {
      ...
      credit : [
        {
          ...
          reasonURL :
              'https://help.ambition.com/hc/en-us/articles/360005048011-How-do-I-set-up-a-Leaderboard-Slide-',
        },
        {
          name : 'Free SVG',
        ...
        },
        {
          ...
          reason : 'Horse Name Data',
          reasonURL :
              'https://horses.lovetoknow.com/horse-names/funny-horse-names',
        },
      ],
    });
  }
  ...
  async run(msg) {
    ...
    await msg.reply(stripIndents`
			**Choose a horse to bet on:** _(Type the number)_
			${
        chosenHorses.map((horse, i) => `**${i + 1}.** ${horse.name}`)
            .join('\n')}
		`);
    const filter = (res) => {
    ...
    return msg.reply(win ? 'Nice job! Your horse won!'
  ...
  async generateLeaderboard(chosenHorses, results) {
    const lb =
        await loadImage(path.join(__dirname, '..', '..', 'assets', 'images',
                                  'horse-race', 'leaderboard.png'));
    const horseImg = await loadImage(path.join(
        __dirname, '..', '..', 'assets', 'images', 'horse-race', 'horse.png'));
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (colors[i])
        drawImageWithTint(ctx, horseImg, colors[i], 37, 114 + 49 * i, 49, 49);
