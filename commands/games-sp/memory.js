...
module.exports = class MemoryCommand extends Command {
  ...
  async run(msg, {level}) {
    ...
    try {
      ...
      const memorizeDisplay =
          memorize.map((word) => `\`${word.toUpperCase()}\``).join(' ');
      ...
      if (answer !== memorizeType)
        return msg.say(`Sorry, you typed it wrong. It was ${memorizeDisplay}.`);
  ...
  genArray(level) {
    const sourceArr =
        [ colors, directions, fruits ][Math.floor(Math.random() * 3)];
    const arr = [];
