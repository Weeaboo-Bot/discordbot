    ...
    try {
      ...
      while (turn < 9) {
        ...
        if (turn === 1) {
          question =
              questions
                  .first[Math.floor(Math.random() * questions.first.length)];
        } else if (turn === 8) {
          question =
              questions.last[Math.floor(Math.random() * questions.last.length)];
        } else {
          const possible =
              questionNums.filter((num) => !blacklist.includes(num));
        ...
        await msg.say(stripIndents`
					**${turn}.** ${question.text}
					${
            answers.map((answer, i) => `- **${choices[i]}.** ${answer.text}`)
                .join('\n')}
				`);
        const filter = (res) => res.author.id === msg.author.id &&
                                choices.slice(0, answers.length)
        ...
        const answer =
            answers[choices.indexOf(choice.first().content.toUpperCase())];
      ...
      return msg.say(stripIndents`
				You are a member of... **${
          houses[houseResult[0]]}**!
				_${descriptions[houseResult[0]]}_

				${
          houseResult
              .map((house) => `${houses[house]}: ${
                       Math.round((points[house] / totalPoints) * 100)}%`)
              .join('\n')}
			`);
    } catch (err) {
      this.client.games.delete(msg.channel.id);
