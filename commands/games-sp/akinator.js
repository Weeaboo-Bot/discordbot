    ...
    try {
      ...
      while (timesGuessed < 3) {
        ...
        await msg.say(stripIndents`
					**${aki.currentStep + 1}.** ${
            aki.question} (${Math.round(Number.parseInt(aki.progress, 10))}%)
					${aki.answers.join(' | ')}${
        ...
        if ((aki.progress >= 90 && !guessResetNum) || forceGuess) {
          ...
          await aki.win();
          const guess =
              aki.answers.filter((g) => !guessBlacklist.includes(g.id))[0];
          if (!guess) {
          ...
          const embed =
              new MessageEmbed()
                  ...
                  .setFooter(forceGuess ? 'Final Guess'
                                        : `Guess ${timesGuessed}`);
      ...
      if (win === 'time')
        return msg.say('I guess your silence means I have won.');
      if (win)
        return msg.say('Bravo, you have defeated me.');
