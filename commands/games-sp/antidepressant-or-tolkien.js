...
module.exports = class AntidepressantOrTolkienCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          'See if you can guess if a word is an Antidepressant or Tolkien character.',
      credit : [
        {
  ...
  async run(msg) {
    ...
    const filter = (res) => {
      ...
      if (drugResponses.includes(choice) || tolkienResponses.includes(choice))
        return true;
      return false;
    };
    ...
    const correct =
        (question.type === 'tolkien' && tolkienResponses.includes(choice)) ||
