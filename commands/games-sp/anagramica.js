...
module.exports = class AnagramicaCommand extends Command {
  ...
  async run(msg, {time}) {
    ...
    try {
      ...
      await msg.reply(stripIndents`
				**You have ${
          time} seconds to provide anagrams for the following letters:**
				${
      ...
      const filter = (res) => {
        if (res.author.id !== msg.author.id)
          return false;
        if (picked.includes(res.content.toLowerCase()))
          return false;
  ...
  async fetchList() {
    ...
    const {body} =
        await request.get(`http://www.anagramica.com/all/${letters.join('')}`);
