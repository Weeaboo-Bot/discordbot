...
registerFont(path.join(__dirname, '..', '..', 'assets', 'fonts',
                       'OPTIKorinna-Agency.otf'),
             {family : 'Korinna'});
...
module.exports = class JeopardyCommand extends Command {
  ...
  async run(msg) {
    ...
    try {
      ...
      const clueCard = await this.generateClueCard(
          question.question.replace(/<\/?i>/gi, ''));
      const connection =
          msg.guild ? this.client.voice.connections.get(msg.guild.id) : null;
      if (connection) {
        connection.play(path.join(__dirname, '..', '..', 'assets', 'sounds',
                                  'jeopardy.mp3'));
        await reactIfAble(msg, this.client.user, '🔉');
      }
      await msg.reply(
          `The category is: **${
              question.category.title.toUpperCase()}**. 30 seconds, good luck.`,
          {
      ...
      if (!msgs.size)
        return msg.reply(`Time's up, the answer was **${answer}**.`);
      const win = msgs.first().content.toLowerCase() === answer.toLowerCase();
      if (!win)
        return msg.reply(`The answer was **${answer}**.`);
  ...
  async fetchQuestion() {
    const {body} =
        await request.get('http://jservice.io/api/random').query({count : 1});
    const data = body[0];
  ...
  async markInvalid(id) {
    const {body} =
        await request.post('http://jservice.io/api/invalid').query({id});
    return body;
  }
