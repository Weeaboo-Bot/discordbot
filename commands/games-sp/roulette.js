...
module.exports = class RouletteCommand extends Command {
      ...
      args : [
        {
          ...
          validate : (space) => {
            if (numbers.includes(Number.parseInt(space, 10)))
              return true;
            if (dozens.includes(space))
              return true;
            if (halves.includes(space))
              return true;
            if (columns.includes(space.toLowerCase()))
              return true;
            if (parity.includes(space.toLowerCase()))
              return true;
            if (colors.includes(space.toLowerCase()))
              return true;
            return oneLine`
  ...
  run(msg, {space}) {
    ...
    return msg.reply(`The result is **${number}${color ? ` ${color}` : ''}**. ${
  ...
  verifyWin(choice, result) {
    ...
    if (parity.includes(choice))
      return parity[result % 2] === choice;
