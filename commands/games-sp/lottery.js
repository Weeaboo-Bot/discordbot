...
module.exports = class LotteryCommand extends Command {
      ...
      args : [
        {
          key : 'choices',
          prompt :
              'What numbers do you choose? Only the first six will be counted.',
          type : 'integer',
          infinite : true,
          max : 70,
          min : 1,
  ...
  run(msg, {choices}) {
    const lotto =
        Array.from({length : 6}, () => Math.floor(Math.random() * 70) + 1);
    const similarities = lotto.filter((num, i) => choices[i] === num).length;
    return msg.reply(stripIndents`
			${lotto.join(', ')}
			You matched **${
        similarities}** numbers, which gives you **${
        prizes[similarities]}**! Congrats!
		`);
  }
};
