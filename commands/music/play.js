  ...
  async run(message, {query, client}) {
    ...
    if (
      ...
      if (message.guild.musicData.isPlaying == false) {
        ...
        return message.say(`Playlist - :musical_note:  ${
    ...
    if (query.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
      ...
      // can be uncommented if you don't want the bot to play videos longer than
      // 1 hour
        ...
        .then(function(response) {
          ...
          youtube.getVideoByID(videos[videoIndex - 1].id)
              .then(function(video) {
                ...
                // can be uncommented if you don't want the bot to play videos
                // longer than 1 hour
                if (video.duration.hours !== 0) {
                  songEmbed.delete();
                  return message.say('I cannot play videos longer than 1 hour');
