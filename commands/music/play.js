...
const youtube = new Youtube(apiKeys.google_token);
...
module.exports = class PlayCommand extends Command {
      ...
      args : [
        {
          ...
          type : 'string',
          validate : function(
              query) { return query.length > 0 && query.length < 200; },
        },
      ],
    });
  }
  ...
  async run(message, {query, client}) {
    ...
    if (!voiceChannel)
      return message.say('Join a channel and try again');
    ...
    if (
      ...
      for (let i = 0; i < videosObj.length; i++) {
        ...
        try {
          ...
          // } else {
          //   return message.say(
          //     `I can't play the full playlist because there will be more than
          //     10 songs in queue`
          //   );
          // }
        } catch (err) {
      ...
      if (message.guild.musicData.isPlaying == false) {
        ...
        return message.say(
            `Playlist - :musical_note:  ${
                playlist.title} :musical_note: has been added to queue`,
        );
      }
    }

    // This if statement checks if the user entered a youtube url, it can be any
    // kind of youtube url
    if (query.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
      ...
      // }
      // // can be uncommented if you don't want the bot to play videos longer
      // than 1 hour if (video.duration.hours !== 0) {
    ...
    message.channel
        .awaitMessages(
            function(msg) {
              return (msg.content > 0 && msg.content < 6) ||
                     msg.content === 'exit';
            },
            {
              max : 1,
        ...
        .then(function(response) {
          ...
          youtube.getVideoByID(videos[videoIndex - 1].id)
              .then(function(video) {
                // // can be uncommented if you don't want the bot to play live
                // streams if (video.raw.snippet.liveBroadcastContent ===
                // 'live') {
                //   songEmbed.delete();
                //   return message.say("I don't support live streams!");
                // }

                // // can be uncommented if you don't want the bot to play
                // videos longer than 1 hour if (video.duration.hours !== 0) {
                //   songEmbed.delete();
                //   return message.say('I cannot play videos longer than 1
                //   hour');
                // }
                ...
                //   songEmbed.delete();
                //   return message.say(
                //     'There are too many songs in the queue already, skip or
                //     wait a bit'
                //   );
                // }
                message.guild.musicData.queue.push(
  ...
  static playSong(queue, message) {
    ...
    const classThis =
        this; // use classThis instead of 'this' because of lexical scope below
    queue[0]
        .voiceChannel.join()
