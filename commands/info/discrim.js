...
module.exports = class DiscrimCommand extends Command {
  ...
  run(message, args) {
    ...
    const users =
        this.client.users.cache.filter((user) => user.discriminator === discrim)
            .map((user) => user.tag);
    if (users.length < 1)
      return message.channel.send(`**${
          message.author.username}**, no users found with the discriminator **${
          discrim}**!`);

    const embed =
        new Discord.MessageEmbed()
            .setTitle(`${users.length} ${
                users.length > 1 ? 'users'
                                 : 'user'} with the discriminator: ${discrim}`)
            .setColor('#4C6684')
