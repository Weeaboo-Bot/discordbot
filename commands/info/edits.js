...
module.exports = class EditsCommand extends Command {
            ...
            args: [
                {
                    key: 'message',
                    prompt:
                        'Please provide the ID of the message you want to find edits for!',
                    type: 'message',
                },
            ],
        });
    ...
    run(message, args) {
            ...
            nestedFields.push(
                `❯\u2000\**${
                    i === m.edits.length - 1
                        ...
                        : i === 0
                        ? 'Latest'
                        : `Edit #${m.edits.length - i - 1}`
                }:**\n` +
                    `•\u2000${
                        m.edits[i].content.length > 0
                            ? truncate(m.edits[i].content, 1024)
                            : '`N/A`'
                    }\n`
            );
        }
        ...
        const embed = new Discord.MessageEmbed()
            .setColor('#B094AF')
            .setAuthor(
                `${m.author.tag} @ ${moment(m.createdAt).format(
                    'MMMM Do YYYY'
                )} (${fromNow(m.editedAt)})`,
                m.author.displayAvatarURL()
            )
            .setFooter(
                `Latest edit: ${moment(m.editedAt).format(
                    'MMMM Do YYYY'
                )} (${fromNow(m.editedAt)})`
            )
