...
module.exports = class UnknownCommandCommand extends Command {
    ...
    run(msg) {
        ...
        const commands = this.makeCommandArray(
            this.client.isOwner(msg.author),
            msg.channel.nsfw
        );
        const command = msg.content.match(
            this.client.dispatcher._commandPatterns[this.client.commandPrefix]
        );
        const str = command ? command[2] : msg.content.split(' ')[0];
        const results = didYouMean(str, commands, {
            returnType: ReturnTypeEnums.ALL_SORTED_MATCHES,
        });
        return msg.reply(stripIndents`
			Unknown command. Use ${this.client.registry.commands
                .get('help')
                .usage()} to view the command list.
    ...
    makeCommandArray(owner, nsfw) {
        ...
        for (const command of this.client.registry.commands.values()) {
            ...
            arr.push(
                ...command.aliases.filter((alias) => !alias.includes('-'))
            );
        }
        return arr;
    }
