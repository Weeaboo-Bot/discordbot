...
module.exports = class HelpCommand extends Command {
  ...
  async run(msg, {command}) {
    if (!command) {
      const embeds = [];
      for (let i = 0; i < Math.ceil(this.client.registry.groups.size / 10);
           i++) {
      ...
      try {
        ...
        if (msg.channel.type !== 'dm')
          msgs.push(await msg.say('📬 Sent you a DM with information.'));
        return msgs;
      } catch {
    ...
    const userPerms =
        command.userPermissions
            ? command.userPermissions.map((perm) => permissions[perm])
                  .join(', ')
            : 'None';
    const clientPerms =
