...
module.exports = class HelpCommand extends Command {
    ...
    async run(msg, { command }) {
        if (!command) {
            ...
            for (
                let i = 0;
                i < Math.ceil(this.client.registry.groups.size / 10);
                i++
            ) {
                const nsfw =
                    msg.channel.nsfw || this.client.isOwner(msg.author);
            ...
            try {
                ...
                if (msg.channel.type !== 'dm') {
                    msgs.push(
                        await msg.say('📬 Sent you a DM with information.')
                    );
                }
                return msgs;
            } catch {
                return msg.reply(
                    'Failed to send DM. You probably have DMs disabled.'
                );
            }
        }
