...
module.exports = class NukeCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          'Bans all members that have joined recently, with new accounts!',
      ...
      args : [
        {
          ...
          label : 'member age',
          prompt :
              'how old (in minutes) should a member be for the nuke to ignore them (server join date)?',
          type : 'float',
          min : 0.1,
          max : 120,
        },
        {
          ...
          label : 'account age',
          prompt :
              "how old (in minutes) should a member's account be for the nuke to ignore them (account age)?",
          type : 'float',
          min : 0.1,
        },
      ],
  ...
  async run(message, {age, join}) {
    ...
    for (const member of members.values()) {
      promises.push(member
                        .send(stripIndents`
					Sorry, but you've been automatically targetted by the cybernuke in the **${
				...
				`)
                        ...
                        .then(() => {
                          ...
                          if (promises.length % 5 === 0) {
                            statusMsg2.edit(`🚀 | Launching cybernuke (${
                                Math.round((promises.length / members.size) *
                                           100)}%)...`);
                          }
                        }));
    }
    ...
    await statusMsg2.edit(
        'Cybernuke impact confirmed. Casualty report incoming...');
    await message.channel.send(
        stripIndents`
            ...
            survivors.length > 0
				...
				${
                      survivors
                          .map((srv) => `**-** ${srv.member.displayName} (${
                                   srv.member.id}): \`${srv.error}\``)
                          .join('\n')}
			`
                : ''}
		`,
