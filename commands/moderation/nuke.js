...
module.exports = class NukeCommand extends Command {
    ...
    async run(message, { age, join }) {
        ...
        const members = message.guild.members.filter(
            (mem) =>
                mem.joinedTimestamp > memberCutoff &&
				...
				`
                    ...
                    .then(() => {
                        ...
                        if (promises.length % 5 === 0) {
                            statusMsg2.edit(
                                `🚀 | Launching cybernuke (${Math.round(
                                    (promises.length / members.size) * 100
                                )}%)...`
                            );
                        }
                    })
        ...
        await message.channel.send(
			...
			${
				...
				${fatalities
                    .map((fat) => `**-** ${fat.displayName} \`(${fat.id})\``)
                    .join('\n')}
			`
                    : 'None'
            }
			...
			${
				...
				${survivors
                    .map(
                        (srv) =>
                            `**-** ${srv.member.displayName} (${srv.member.id}): \`${srv.error}\``
                    )
                    .join('\n')}
			`
                    : ''
