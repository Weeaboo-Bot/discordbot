...
module.exports = class SoftbanCommand extends Command {
    ...
    async run(message, {memberName,reason}) {
        ...
        if (member.roles.highest.position > message.member.roles.highest.position - 1) return message.channel.send(`❎ | You can't softban **${member.user.username}**! Their position is higher than you!`);
        if (!member.bannable) return message.channel.send(`❎ | I can't softban **${member.user.username}**! Their role is higher than mine!`);
