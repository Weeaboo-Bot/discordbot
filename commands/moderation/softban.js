...
module.exports = class SoftbanCommand extends Command {
  ...
  async run(message, {memberName, reason}) {
    ...
    if (member.roles.highest.position >
      ...
      return message.channel.send(`❎ | You can't softban **${
          member.user.username}**! Their position is higher than you!`);
    if (!member.bannable)
      return message.channel.send(`❎ | I can't softban **${
    ...
    try {
      await member.send(
          `You were softbanned from **${message.guild.name}** by **${
              message.author.tag}**!\n\**Reason:** ${reason}`);
    } catch (err) {
      await message.channel.send(
          `❎ | **${message.author.username}**, failed to Send DM **${
              member}** the reason! \`${err}\``);
    }
