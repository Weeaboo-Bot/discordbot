...
const ImageRegex =
    /(?:([^:/?#]+):)?(?:\/\/([^\/?#]*))?([^?#]*\.(?:png|jpe?g|gifv?|webp|bmp|tiff|jfif))(?:\?([^#]*))?(?:#(.*))?/gi;
const LinkRegex =
...
module.exports = class PruneCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      examples : [
        '~prune [1-100] <all/images/links/bots/codeblocks/attachments/embeds/me>',
      ],
      throttling : {
        usages : 1,
        duration : 10,
  ...
  async run(message, args) {
    ...
    if (type == 'all') {
      try {
        ...
        return message.channel.send(`🍇 | **${
            message.author.username}**, successfully pruned ${messages.size} ${
            messages.size == 1 ? 'message!' : 'messages!'}`);
      } catch (err) {
        console.log(err);
        return message.channel.send(
            '❎ | These messages are too old to be deleted! I can only delete messages within two weeks!');
      }
    }
    ...
    if (type == 'images' || type == 'pics' || type == 'image') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no images to prune in the last ${
              count} messages!`);

        await message.channel.bulkDelete(flushable);

        const m = await message.channel.send(`🍇 | **${
            message.author.username}**, successfully pruned **${
            flushable.size}** ${flushable.size == 1 ? 'image!' : 'images!'}`);
    ...
    if (type == 'bots' || type == 'bot') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no bot messages to prune in the last ${
              count} messages!`);

        const m = await message.channel.send(
            `🍇 | **${message.author.username}**, successfully pruned **${
                flushable.size}** ${
                flushable.size == 1 ? 'bot message!' : 'bot messages!'}`);
    ...
    if (type == 'codeblocks' || type == 'code') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no codeblocks to prune in the last ${
              count} messages!`);
        ...
        const m = await message.channel.send(
            `🍇 | **${message.author.username}**, successfully pruned **${
                flushable.size}** ${
                flushable.size == 1 ? 'codeblock!' : 'codeblocks!'}`);
    ...
    if (type == 'attachments' || type == 'attachment' || type == 'files' ||
        type == 'file') {
      try {
        const messages = await message.channel.messages.fetch({
          limit : count,
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no attachments to prune in the last ${
              count} messages!`);
        ...
        const m = await message.channel.send(
            `🍇 | **${message.author.username}**, successfully pruned **${
                flushable.size}** ${
                flushable.size == 1 ? 'attachment!' : 'attachments!'}`);
    ...
    if (type == 'embeds' || type == 'embed') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no embeds to prune in the last ${
              count} messages!`);

        await message.channel.bulkDelete(flushable);
        const m = await message.channel.send(`🍇 | **${
            message.author.username}**, successfully pruned **${
            flushable.size}** ${flushable.size == 1 ? 'embed!' : 'embeds!'}`);
    ...
    if (type == 'me') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no messages from you to prune in the last ${
              count} messages!`);

        await message.channel.bulkDelete(flushable);
        const m = await message.channel.send(
            `🍇 | **${message.author.username}**, successfully pruned **${
                flushable.size}** of your messages!`);
    ...
    if (type == 'link' || type == 'links') {
      try {
        ...
        if (flushable.size == 0)
          return message.channel.send(`🍇 | **${
              message.author
                  .username}**, there were no links to prune in the last ${
              count} messages!`);

        await message.channel.bulkDelete(flushable);
        const m = await message.channel.send(
            `🍇 | **${message.author.username}**, successfully pruned **${
                flushable.size}** ${flushable.size == 1 ? 'link!' : 'links!'}`);
