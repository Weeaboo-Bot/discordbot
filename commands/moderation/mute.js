...
module.exports = class MuteCommand extends Command {
  ...
  async run(message, {memberName}) {
    ...
    if (member.roles.highest.position >
      ...
      return message.channel.send(`❎ | You can't mute **${
          member.user.username}**! Their position is higher than you!`);
    if (!member.manageable)
      return message.channel.send(`❎ | I can't mute **${
          member.user.username}**! Their role is higher than mine!`);

    await message.channel.send(
        `Are you sure you want to mute **${member.user.tag}** in **${
            message.guild.channels.cache.find(
                (channel) =>
                    channel.id === message.channel.id)}**? \`\`(y/n)\`\``);
    ...
    try {
      await message.guild.channels.cache.get(message.channel.id)
      ...
      await message.channel.send(`❎ | **${
          message.author.username}**, there was an error trying to mute **${
          member}**! \`${err}\``);
    }

    return await message.channel.send(
        `**${message.author.username}**, successfully muted **${
            member.user.tag}** in **${
