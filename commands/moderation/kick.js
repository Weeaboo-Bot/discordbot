...
module.exports = class KickCommand extends Command {
  ...
  async run(message, {memberName, reason}) {
    ...
    if (member.roles.highest.position >
      ...
      return message.channel.send(`❎ | You can't kick **${
          member.user.username}**! Their position is higher than you!`);
    if (!member.kickable)
      return message.channel.send(`❎ | I can't kick **${
    ...
    try {
      await member.send(`You were kicked from **${message.guild.name}** by **${
          message.author.tag}**!\n\**Reason:** ${reason}`);
    } catch (err) {
      await message.channel.send(
          `❎ | **${message.author.username}**, failed to Send DM to **${
              member}**! \`${err}\``);
    }

    await member.kick(`${message.author.tag}: ${reason}`);
    return message.channel.send(
        `**${message.author.username}**, successfully kicked **${
            member.user.tag}**! 👋`);
  }
};
