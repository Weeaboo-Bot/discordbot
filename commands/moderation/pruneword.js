...
module.exports = class PruneWordCommand extends Command {
  ...
  async run(message, args) {
    ...
    try {
      ...
      const flushable =
          messages.filter((m) => m.content.toLowerCase().includes(inc));
      if (flushable.size == 0)
        return message.channel.send(`🍇 | **${
            message.author
                .username}**, there were no messages containing the word **${
            inc}** in the last ${count} messages!`);
      await message.channel.bulkDelete(flushable);

      const m = await message.channel.send(`🍇 | **${
          message.author.username}**, successfully pruned ${flushable.size} ${
          flushable.size == 1 ? `message containing the word **${inc}**!`
                              : `messages containing the word **${inc}**!`}`);
