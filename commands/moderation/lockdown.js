    ...
    if (type === 'start') {
      await message.guild.channels.cache.get(message.channel.id)
          .overwritePermissions(
              [
                {
                  id : message.guild.roles.cache.find((role) => role.name ===
                                                                'Weaboos'),
                  deny : [ 'SEND_MESSAGES' ],
                },
              ],
              `Lockdown started by ${message.author.tag}`);
      return message.channel.send(`Lockdown has initiated! ${
          message.guild.roles.cache.find((role) => role.name === 'Weaboos')
              .name} are now unable to send a message in this channel!\n\Please use \`lockdown stop\` to end the lockdown!`);
    } else if (type === 'stop') {
      await message.guild.channels.cache.get(message.channel.id)
          .overwritePermissions(
              [
                {
                  id : message.guild.roles.cache
                           .find((role) => role.name === 'Weaboos')
                           .id,
                  allow : [ 'SEND_MESSAGES' ],
                },
              ],
