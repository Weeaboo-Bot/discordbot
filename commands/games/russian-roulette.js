...
module.exports = class RussianRouletteCommand extends Command {
  ...
  async run(msg, {opponent}) {
    ...
    try {
      ...
      let userTurn = true;
      const gun =
          shuffle([ true, false, false, false, false, false, false, false ]);
      let round = 0;
      let winner = null;
      let quit = false;
      while (!winner) {
        ...
        if (gun[round]) {
          ...
          await msg.say(stripIndents`
              ...
              8 - round - 1} shots left)
					`);
          const keepGoing =
              await verify(msg.channel, opponent.bot ? msg.author : notPlayer);
          if (!keepGoing) {
            if (opponent.bot)
              winner = opponent;
            else
      ...
      this.client.games.delete(msg.channel.id);
      if (quit)
        return msg.say(`${winner} wins, because their opponent was a coward.`);
