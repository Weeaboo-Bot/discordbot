...
const {awaitPlayers} = require('../../helpers/functions');
...
module.exports = class BingoCommand extends Command {
    ...
    try {
      ...
      while (!winner) {
        ...
        for (const player of players.values()) {
          try {
            await player.user.send(stripIndents`
							**${
                this.findRowValue(
                    picked)} ${picked}** was called in ${msg.channel}.
							${
        ...
        await msg.say(stripIndents`
            ...
            validNums.length - 1} numbers left._
				`);
        const filter = res => {
          ...
          if (!this.checkBingo(players.get(res.author.id).board, called)) {
            msg.say(`${res.author}, you don't have bingo, liar.`)
                .catch(() => null);
            return false;
          }
          return true;
        ...
        const bingo =
            await msg.channel.awaitMessages(filter, {max : 1, time : 20000});
        if (!players.size) {
          winner = 0;
          break;
        }
      ...
      this.client.games.delete(msg.channel.id);
      if (winner === 0)
        return msg.say('Everyone dropped out...');
      if (!winner)
        return msg.say('I called the entire board, but no one called bingo...');
  ...
  findRowValue(num) {
    if (nums.B.includes(num))
      return 'B';
    if (nums.I.includes(num))
      return 'I';
    if (nums.N.includes(num))
      return 'N';
    if (nums.G.includes(num))
      return 'G';
    if (nums.O.includes(num))
      return 'O';
    return null;
  }

  checkLine(called, a, b, c, d, e) {
    return called.includes(a) && called.includes(b) && called.includes(c) &&
           called.includes(d) && called.includes(e);
  }

  checkBingo(bd, ca) {
    for (let r = 0; r < rows.length; r++) {
      if (this.checkLine(ca, bd[r][0], bd[r][1], bd[r][2], bd[r][3], bd[r][4]))
        return true;
    }
    for (let c = 0; c < rows.length; c++) {
      if (this.checkLine(ca, bd[0][c], bd[1][c], bd[2][c], bd[3][c], bd[4][c]))
        return true;
    }
    if (this.checkLine(ca, bd[0][0], bd[1][1], bd[2][2], bd[3][3], bd[4][4]))
      return true;
    if (this.checkLine(ca, bd[4][0], bd[3][1], bd[2][2], bd[1][3], bd[0][4]))
      return true;
    return false;
  }
};