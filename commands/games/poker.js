...
module.exports = class PokerCommand extends Command {
  ...
  async run(msg, { playersCount }) {
    ...
    try {
      ...
      const { players, deck, turnData } = this.client.games.get(
        msg.channel.id
      ).data;
      ...
      while (!winner) {
        ...
        let keepGoing = await this.gameRound(
          msg,
          players,
          deck,
          folded,
        ...
        await msg.say(stripIndents`
				...
				`);
        await delay(10000);
        keepGoing = await this.gameRound(
          msg,
          players,
          deck,
          folded,
  ...
  makeTurnRotation(players, folded, bigBlind, smallBlind) {
    return [
      smallBlind.id,
      ...players
        .filter((p) => bigBlind.id !== p.id && smallBlind.id !== p.id)
        .map((p) => p.id),
      bigBlind.id,
  ...
  async gameRound(msg, players, deck, folded, turnData, bigBlind, smallBlind) {
    let turnOver = false;
    const turnRotation = this.makeTurnRotation(
      players,
      folded,
      bigBlind,
      smallBlind
  ...
  async bettingRound(msg, players, turnRotation, folded, data) {
    ...
    const filter = (res) => {
      ...
      if (choice.startsWith("raise")) {
        ...
        if (amount + data.currentBet > turnPlayer.money || amount < 1) {
          ...
          res
            .reply(`You can only bet up to $${formatNumber(maxBet)}!`)
            .catch(() => null);
          return false;
        }
        return true;
    ...
    const raiseValue = raiseRegex.test(choiceAction)
    ...
    return (
      (oldHighestBetter.id === turnPlayer.id &&
        choiceAction === "check" &&
