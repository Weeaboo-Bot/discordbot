...
module.exports = class PokerCommand extends Command {
  ...
  async run(msg, {playersCount}) {
    ...
    try {
      ...
      const {players, deck, turnData} =
          this.client.games.get(msg.channel.id).data;
      ...
      while (!winner) {
        ...
        for (const player of players.values()) {
          ...
          try {
            ...
            await msg.say(
                `${player.user}, I couldn't send your hand! Turn on DMs!`);
          }
        }
        turnData.pot = bigBlindAmount + smallBlindAmount;
        turnData.currentBet = bigBlindAmount;
        turnData.highestBetter = bigBlind;
        let keepGoing = await this.gameRound(msg, players, deck, folded,
                                             turnData, bigBlind, smallBlind);
        if (!keepGoing) {
          if (players.size < 2) {
        ...
        await msg.say(stripIndents`
            ...
            dealerHand.map((card) => card.textDisplay).join('\n')}
				...
				`);
        await delay(10000);
        keepGoing = await this.gameRound(msg, players, deck, folded, turnData,
                                         bigBlind, smallBlind);
        if (!keepGoing) {
          if (players.size < 2) {
            winner = players.first();
        ...
        if (winners.length > 1) {
          await msg.say(stripIndents`
              ...
              list(winners.map((w) => `**${w.user.user}**`))}.
						${
              winners.map(winner.descr).join(', ')}

						__**Results**__
						${
              solved.map((solve) => `${solve.user.user.tag}: ${solve.descr}`)
                  .join('\n')}
  ...
  determineActions(turnPlayer, currentBet) {
    const actions = [];
    if (turnPlayer.currentBet !== currentBet)
      actions.push('fold');
    if (turnPlayer.money > currentBet)
      actions.push('raise <amount>');
    if (turnPlayer.money >= currentBet && turnPlayer.currentBet !== currentBet)
      actions.push('call');
    if (currentBet === turnPlayer.currentBet)
      actions.push('check');
    return actions;
  }
  ...
  makeTurnRotation(players, folded, bigBlind, smallBlind) {
    return [
      smallBlind.id,
      ...players.filter((p) => bigBlind.id !== p.id && smallBlind.id !== p.id)
          .map((p) => p.id),
      bigBlind.id,
  ...
  async gameRound(msg, players, deck, folded, turnData, bigBlind, smallBlind) {
    let turnOver = false;
    const turnRotation =
        this.makeTurnRotation(players, folded, bigBlind, smallBlind);
    while (!turnOver)
      turnOver =
          await this.bettingRound(msg, players, turnRotation, folded, turnData);
  ...
  async bettingRound(msg, players, turnRotation, folded, data) {
    ...
    await msg.say(stripIndents`
			**Pot: $${formatNumber(data.pot)}**
			_Highest Bet: $${formatNumber(data.currentBet)} (${
        data.highestBetter.user.tag})_

			${turnPlayer.user}, what do you want to do? You can ${
        displayActions}.
		`);
    const filter = (res) => {
      if (res.author.id !== turnPlayer.id)
        return false;
      let choice = res.content.toLowerCase();
      if (actions.includes(choice) && !choice.startsWith('raise'))
        return true;
      if (choice.startsWith('raise')) {
        if (!raiseRegex.test(choice))
          return false;
        ...
        if (amount + data.currentBet > turnPlayer.money || amount < 1) {
          ...
          res.reply(`You can only bet up to $${formatNumber(maxBet)}!`)
              .catch(() => null);
          return false;
        }
        return true;
    ...
    const raiseValue =
        raiseRegex.test(choiceAction)
            ? Number.parseInt(choiceAction.match(raiseRegex)[1], 10)
            : null;
    if (raiseValue) {
      const amountChange =
          raiseValue + (data.currentBet - turnPlayer.currentBet);
      ...
      await msg.say(
          `${turnPlayer.user} **raises $${formatNumber(raiseValue)}**.`);
      ...
      data.pot += amountChange;
      await msg.say(
          `${turnPlayer.user} **calls $${formatNumber(data.currentBet)}**.`);
    ...
    if (choiceAction !== 'fold')
      turnRotation.push(turnRotation[0]);
    turnRotation.shift();
    turnPlayer.hasGoneOnce = true;
    const nextPlayer = players.get(turnRotation[0]);
    return ((oldHighestBetter.id === turnPlayer.id &&
             choiceAction === 'check' && nextPlayer.hasGoneOnce) ||
