...
module.exports = class GuesspionageCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          'Tests your knowledge of humans as you guess how people responded to poll questions.',
      guildOnly : true,
      credit : [
        {
          name : 'Jackbox Games',
        ...
        },
        {
          ...
          reasonURL :
              'https://www.playstationtrophies.org/game/the-jackbox-party-pack-3/trophy/157520-Guesspionage--Perfect-Surveillance.html',
        },
        {
          ...
          reason : 'Question Data',
          reasonURL :
              'https://www.trueachievements.com/forum/viewthread.aspx?tid=850920',
        },
      ],
      args : [
        {
    ...
    try {
      ...
      while (userTurn.length) {
        ...
        const valid = questions.filter(
            (question) => !used.includes(question.text) &&
                          (msg.channel.nsfw ? true : !question.nsfw));
        ...
        const initialGuessFilter = (res) => {
          if (res.author.id !== mainUser.id)
            return false;
          const int = Number.parseInt(res.content, 10);
          return int >= 0 && int <= 100;
        };
        const initialGuess =
            await msg.channel.awaitMessages(initialGuessFilter, {
              max : 1,
              time : 30000,
            });
        ...
        const everyoneElseFilter = (res) => {
          if (res.author.id === mainUser.id)
            return false;
          if (guessed.includes(res.author.id))
            return false;
        ...
        const everyoneElse =
            await msg.channel.awaitMessages(everyoneElseFilter, {
        ...
        const higherLower =
            everyoneElse.map((res) => ({
                               guess : res.content.toLowerCase(),
                               id : res.author.id,
                             }));
        for (const answer of higherLower) {
          ...
          if (uGuess === 'higher' && guess < question.answer) {
            ...
            pts.get(answer.id).points += 1000;
          } else if (uGuess === 'much higher' && guess < question.answer &&
                     question.answer - guess >= 15) {
            pts.get(answer.id).points += 2000;
          } else if (uGuess === 'much lower' && guess > question.answer &&
                     guess - question.answer >= 15) {
            pts.get(answer.id).points += 2000;
          }
        }
  ...
  makeLeaderboard(pts) {
    ...
    return pts.sort((a, b) => b.points - a.points).map((player) => {
