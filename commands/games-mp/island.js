...
module.exports = class IslandCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      memberName : 'island',
      description :
          'Who will be the final two left on the island after a series of vote-kicks?',
      guildOnly : true,
      args : [
        {
          key : 'playersCount',
    ...
    try {
      ...
      while (playersLeft.size > 2) {
        ...
        await msg.say(stripIndents`
					**Day ${
            turn}.** Who should be kicked off the island?
        ...
        await msg.say(stripIndents`
					...
					${
            choices.map((player, i) => `**${i + 1}.** ${player.user.tag}`)
                .join('\n')}
				`);
        const votes = new Collection();
        const voteFilter = (res) => {
          if (!playersLeft.has(res.author.id))
            return false;
        ...
        const kicked =
            players.get(votes.sort((a, b) => b.votes - a.votes).first().id);
      ...
      return msg.say(`Congrats, ${
          winners.map((player) => player.user.tag).join(' and ')}!`);
    } catch (err) {
      this.client.games.delete(msg.channel.id);
      throw err;
    }
