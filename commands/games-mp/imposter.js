...
module.exports = class ImposterCommand extends Command {
  ...
  async run(msg, {playersCount}) {
    ...
    try {
      ...
      const imposter =
          awaitedPlayers[Math.floor(Math.random() * awaitedPlayers.length)];
      ...
      while (players.filter((player) => !player.killed).size > 2) {
        ...
        await msg.say(`There are **${
            playersLeft}** players left. Talk until someone says the kill word.`);
        const filter = (res) => {
          const player = players.get(res.author.id);
          if (!player || player.killed || player.imposter)
            return false;
          if (res.content && wordRegex.test(res.content))
            return true;
          return false;
        };
        ...
        if (kicked.id === players.find((player) => player.imposter).id) {
          ...
          winners.push(...players.filter((player) => !player.killed)
