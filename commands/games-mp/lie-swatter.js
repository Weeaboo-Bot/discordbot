...
module.exports = class LieSwatterCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          "Players are given a fact and must quickly decide if it's True or a Lie.",
      credit : [
        {
          name : 'Jackbox Games',
  ...
  async run(msg, {players}) {
    ...
    try {
      ...
      while (questions.length) {
        ...
        if (!msgs.size) {
          await msg.say(`No answers? Well, it was ${
              question.answer ? 'true' : 'a lie'}.`);
          if (lastTurnTimeout) {
            break;
          } else {
        ...
        const answers = msgs.map((res) => {
          let answer;
          if (trueOptions.includes(res.content.toLowerCase()))
            answer = true;
          else if (falseOptions.includes(res.content.toLowerCase()))
            answer = false;
          return {answer, id : res.author.id};
        });
        const correct =
            answers.filter((answer) => answer.answer === question.answer);
        ...
        await msg.say(stripIndents`
            ...
            question.answer ? 'true' : 'a lie'}**!
					...
					_Fastest Guess: ${
            correct.length ? `${pts.get(correct[0].id).user.tag} (+75 pts)`
                           : 'No One...'}_
