...
module.exports = class PickANumberCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          "Two players pick a number between 1 and 10. Whoever's closer wins.",
      args : [
        {
          key : 'opponent',
  ...
  async run(msg, {opponent}) {
    ...
    try {
      ...
      if (opponent.bot) {
        ...
        await msg.say(`Okay, ${clientOpp ? 'I' : `${opponent}`} pick${
            clientOpp ? '' : 's'} ${player2Pick}!`);
      } else {
        userTurn = false;
        await msg.say(
            `${opponent}, pick a number from 1 to 10, except ${player1Pick}.`);
      ...
      const winNum = [ player1Pick, player2Pick ].sort(
          (a, b) => Math.abs(num - a) - Math.abs(num - b))[0];
      const winner = winNum === player1Pick ? msg.author : opponent;
      const clientWin = clientOpp && this.client.user.id === winner.id;
      this.client.games.delete(msg.channel.id);
      return msg.say(`The number was **${num}**! ${
          clientWin ? 'I' : winner} win${clientWin ? '' : 's'}!`);
    } catch (err) {
      this.client.games.delete(msg.channel.id);
      throw err;
    }
