  ...
  async run(msg, {opponent}) {
    ...
    try {
      ...
      while (turn < 10) {
        ...
        const filter =
            (res) => [msg.author.id, opponent.id].includes(res.author.id) &&
                     emoji.includes(res.content);
      ...
      this.client.games.delete(msg.channel.id);
      if (aPts === oPts)
        return msg.say("It's a tie!");
      const userWin = aPts > oPts;
      return msg.say(`You win ${userWin ? msg.author : opponent} with ${
          userWin ? aPts : oPts} points!`);
    } catch (err) {
      this.client.games.delete(msg.channel.id);
      throw err;
    }
