...
module.exports = class DotsAndBoxesCommand extends Command {
  ...
  async run(msg, {opponent}) {
    ...
    try {
      ...
      while (taken.length < 40) {
        ...
        if (newSquares.length) {
          for (const newSquare of newSquares) {
            if (userTurn)
              userOwned.push(newSquare);
            else
              oppoOwned.push(newSquare);
          }
          if (taken.length < 40) {
            await msg.say(`${
                user}, great job! Keep going until you can't make any more!`);
          }
        } else {
          userTurn = !userTurn;
        }
      ...
      return msg.say(winner ? `Congrats, ${winner}!`
  ...
  calcNewSquare(taken, userOwned, oppoOwned) {
    ...
    for (const square of squareIDs) {
      if (userOwned.includes(square) || oppoOwned.includes(square))
        continue;
      if (this.calcSquare(square, taken))
    ...
    board.forEach((values, row) => {
      if (row !== 0) {
        ...
        for (let i = 0 + row * 5; i < 5 + row * 5; i++) {
          if (taken.includes(`${i - 5}-${i}`))
            takenMids += '||';
          else
            takenMids += '  ';
          takenMids += userOwned.includes(i)
                           ? 'P1'
                           : oppoOwned.includes(i) ? 'P2' : '  ';
        }
        takenMids += '█';
