...
module.exports = class SpamWarCommand extends Command {
    ...
    async run(msg, { opponent }) {
        ...
        try {
            ...
            const winnerPts =
                authorMsgs > opponentMsgs ? authorMsgs : opponentMsgs;
            const loserPts =
                authorMsgs > opponentMsgs ? opponentMsgs : authorMsgs;
            this.client.games.delete(msg.channel.id);
            if (authorMsgs === opponentMsgs) {
                return msg.say(
                    `It was a tie! What are the odds? You both got **${winnerPts}** points!`
                );
            }
            return msg.say(
                `The winner is ${winner}, with **${winnerPts}** points! Your opponent only got ${loserPts}...`
            );
        } catch (err) {
            this.client.games.delete(msg.channel.id);
            throw err;
