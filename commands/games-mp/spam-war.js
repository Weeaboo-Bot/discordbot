...
module.exports = class SpamWarCommand extends Command {
  ...
  async run(msg, {opponent}) {
    if (opponent.bot)
      return msg.reply('Bots may not be played against.');
    ...
    try {
      ...
      await msg.say(
          'You get one point per character in your messages. You get 1 minute to spam.');
      await delay(5000);
      await msg.say('You have **1 minute** to spam. Go!');
      const msgs = await msg.channel.awaitMessages(
          (res) => [opponent.id, msg.author.id].includes(res.author.id), {
            time : 60000,
          });
      const authorMsgs =
          msgs.filter((authorMsg) => authorMsg.author.id === msg.author.id)
              .reduce((a, b) => a + b.content.length, 0);
      const opponentMsgs =
          msgs.filter((opponentMsg) => opponentMsg.author.id === opponent.id)
              .reduce((a, b) => a + b.content.length, 0);
      ...
      if (authorMsgs === opponentMsgs) {
        return msg.say(`It was a tie! What are the odds? You both got **${
            winnerPts}** points!`);
      }
      return msg.say(`The winner is ${winner}, with **${
          winnerPts}** points! Your opponent only got ${loserPts}...`);
    } catch (err) {
      this.client.games.delete(msg.channel.id);
      throw err;
    }
