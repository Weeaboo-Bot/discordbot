...
module.exports = class ClipboardCommand extends Command {
  ...
  async run(message, {action}) {
    ...
    case 'copy': {
      ...
      message.reply(
          'You will now have 15 seconds to send the message to be copied.');
      ...
      message.channel
          ...
          .then(collected => {
            if (collected.first().author.id !== message.author.id) {
              message.reply(
                  'Sorry, only the orignal command issuer can use this!');
            }
            ...
            clipboard.write(collected.first().cleanContent)
                .then(
                    res => {

                        message.reply(
                            'Thanks, your text has been copied to your local clipboard!')})
          })
          .catch(() => {
    ...
    case 'paste': {
      const optionList = [ 'DM', 'Current Channel' ]
      message.reply(
          `Please select where I should paste this to: ${optionList}`);
      message.channel
          .awaitMessages(m => optionList.includes(m), {max : 1, time : 15000})
          .then(collected => {
            // only accept messages by the user who sent the command
            ...
            while (optionList.includes(currMsg)) {
              if (currMsg == 'DM') {
                ...
                message.client.users.fetch(id).then(
                    user => {user.send(clipboard.readSync())});
