...
module.exports = class ShutdownCommand extends Command {
  ...
  async run(msg, {code}) {
    if (this.client.games.size > 0) {
      await msg.reply(`There are currently **${
          this.client.games.size}** games going on. Are you sure?`);
      const verification = await verify(msg.channel, msg.author);
      if (!verification)
        return msg.say('Aborted restart.');
    }
    if (this.client.phone.size > 0) {
      await msg.reply(`There are currently **${
          this.client.phone.size}** phone calls going on. Are you sure?`);
      const verification = await verify(msg.channel, msg.author);
      if (!verification)
        return msg.say('Aborted restart.');
    }
