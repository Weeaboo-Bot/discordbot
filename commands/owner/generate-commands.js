...
module.exports = class GenerateCommandsCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description : 'Generates the commands list for Weaboo Bots\'s README.',
  ...
  async run(msg) {
    ...
    const text =
        `Total: ${this.client.registry.commands.size}\n${list.join('\n')}`;
    await msg.direct(
        {files : [ {attachment : Buffer.from(text), name : 'commands.txt'} ]});
