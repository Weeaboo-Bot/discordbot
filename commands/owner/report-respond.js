...
module.exports = class ReportRespondCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      args : [
        ...
        {
          key : 'type',
          prompt : `What is the reason for your report? Either ${
              list(types, 'or')}.`,
          type : 'string',
          oneOf : types,
  ...
  async run(msg, {user, type, message}) {
    ...
    try {
      await user.send(
          'Your report has been evaluated with the following message:',
          {embed});
      return msg.say(`${displaytypes[type]} sent to ${user.tag}.`);
    } catch {
      return msg.say(`Could not send ${displaytypes[type]} to ${
          user.tag}. Probably blocked me.`);
    }
  }
};
