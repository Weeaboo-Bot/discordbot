...
module.exports = class GenerateCreditCommand extends Command {
  ...
  async run(msg) {
    ...
    for (const command of this.client.registry.commands.values()) {
      if (!command.credit || command.credit.length <= 1)
        continue;
    ...
    const mapped = credit.map(
        (c) => `- ${embedURL(c.name, c.url || '', false)}\n${
            sortByName(c.commands, 'name')
                .map((cmd) => {
                  if (!cmd.reasonURL)
                    return `	* ${cmd.name} (${cmd.reason})`;
                  return `	* ${cmd.name} (${
                      embedURL(cmd.reason, cmd.reasonURL || '', false)})`;
                })
                .join('\n')}`);
    await msg.direct({
      files : [
        {
          attachment : Buffer.from(mapped.join('\n')),
          name : 'credit.txt',
        },
      ],
    });
