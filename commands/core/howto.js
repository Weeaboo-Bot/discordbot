    ...
    async run(message) {
        if (message.channel.nsfw) {
            return message.channel.send(
                "Looks like you've got it all set up already! An NSFW channel perfect for...lewding? Try `~nsfwcommands` to see what I can do!"
            );
        }
        ...
        try {
            ...
            await message.channel.send(
                `❎ | **${message.author.username}**, there was an error trying to make this channel into an NSFW channel! \`${err}\``
            );
        }

        return await message.channel.send(
            `✅ | **${message.author.username}**, successfully made **${message.channel.name}** into an NSFW channel!`
        );
    }
};
