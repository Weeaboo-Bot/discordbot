...
module.exports = class NomCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (args.length < 1) {
      ...
      return message.channel.send(`${message.author} noms on... themselves..?`,
                                  {embed : embed});
          ...
          .then(function(response) {
            ...
            const embed3 =
                new Discord.MessageEmbed().setColor('#FBCFCF').setImage(
                    `https://rra.ram.moe${response.data.path}`);
            return message.channel.send('Nyaa~ s-senpai... (´Å`∗)... ',
                                        {embed : embed3});
          })
      ...
      await axios.get('https://rra.ram.moe/i/r?type=nom')
          .then(function(response) {
            ...
            return message.channel.send(
                `${message.author} noms on ${recipient}!`, {embed : embed4});
          })
          .catch(function(error) {
            // handle error
            console.log(error);
