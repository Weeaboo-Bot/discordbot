...
module.exports = class PatCommand extends Command {
  ...
  run(message) {
    ...
    if (!recipient) {
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(pat);
      return message.channel.send(
          `${
              message
                  .author}, you can't pat yourself, but I'll pat you! (´꒳\`)`,
          {embed : embed});
      ...
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(pat);
      return message.channel.send(`${message.author} pats ${recipient}!`,
                                  {embed : embed});
    }
  }
};
