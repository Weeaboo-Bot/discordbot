...
module.exports = class SlapCommand extends Command {
  ...
  run(message) {
    ...
    if (!recipient) {
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(selfSlap());
      return message.channel.send(
          `${message.author}, please don't slap yourself!`, {embed : embed});
      ...
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(slap);
      return message.channel.send(`${message.author} slaps ${recipient}!`,
                                  {embed : embed});
    }
  }
};
