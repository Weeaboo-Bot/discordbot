...
module.exports = class HugCommand extends Command {
  ...
  run(message) {
    ...
    if (!recipient) {
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(hug);
      ...
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(hug);
      return message.channel.send(
          'ల(*´= ◡ =｀*) Such a warm hug..thank you~~ Nyaa~~', {embed : embed});
    } else {
      const embed =
          new Discord.MessageEmbed().setColor('#FBCFCF').setImage(hug);
      return message.channel.send(`${message.author} hugs ${recipient}!`,
                                  {embed : embed});
    }
  }
};
