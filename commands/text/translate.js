...
const codes =
    Object.keys(translate.languages)
        .filter((code) => typeof translate.languages[code] !== 'function');
...
module.exports = class TranslateCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      args : [
        ...
        {
          key : 'target',
          prompt : `Which language would you like to translate to? Either ${
              list(codes, 'or')}.`,
          type : 'string',
          validate : (target) => {
        ...
        {
          key : 'base',
          prompt : `Which language would you like to use as the base? Either ${
              list(Object.keys(codes), 'or')}.`,
          type : 'string',
          default : 'auto',
          validate : (base) => {
            if (translate.languages.isSupported(base))
              return true;
  ...
  async run(msg, {text, target, base}) {
    try {
      const {text : result, from} = await translate(text, {
        to : target,
        from : base,
      });
      const embed =
          new MessageEmbed()
              ...
              .setFooter('Powered by Google Translate',
                         'https://i.imgur.com/h3RoHyp.png')
              .addField(`❯ From: ${translate.languages[from.language.iso]}`,
                        from.text.value || text)
