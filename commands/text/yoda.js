...
module.exports = class YodaCommand extends Command {
  ...
  async run(msg, {sentence}) {
    try {
      const {body} =
          await request.get('https://yoda-api.appspot.com/api/v1/yodish')
              .query({text : sentence});
      if (!body.yodish)
        return msg.reply('Empty, this message is. Try again later, you must.');
      return msg.say(body.yodish);
    } catch (err) {
      return msg.reply(`Being a jerk again, Yoda is: \`${
          err.message}\`. Try again later, you must.`);
    }
  }
};
