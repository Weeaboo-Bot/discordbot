...
module.exports = class FinalGradeCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description :
          'Determines the grade you need to make on your final to get your desired course grade.',
      credit : [
        {
  ...
  run(msg, {current, desired, weight}) {
    const required =
        Math.round(((desired / 100 - (current / 100) * (1 - weight / 100)) /
                    (weight / 100)) *
                   100);
    ...
    return msg.say(`You will need to score at least ${
        required}% on your final to get a ${desired}% overall. ${text}`);
  }
};
