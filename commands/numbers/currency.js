...
module.exports = class CurrencyCommand extends Command {
    ...
    super(client, {
      ...
      args : [
        ...
        },
        {
          key : 'base',
          prompt :
              'What currency code (e.g. USD) do you want to use as the base?',
          type : 'string',
          min : 3,
          max : 3,
  ...
  async run(msg, {base, target, amount}) {
    ...
    try {
      ...
      return msg.say(`${formatNumber(amount)} ${base} is ${
          formatNumber(amount * rate)} ${target}.`);
    } catch (err) {
      if (err.status === 400)
        return msg.say('Invalid base/target.');
      return msg.reply(
  ...
  async fetchRate(base, target) {
    ...
    if (this.rates.has(query))
      return this.rates.get(query);
    const {body} =
        await request.get('https://api.exchangeratesapi.io/latest').query({
          base,
          symbols : target,
        });
