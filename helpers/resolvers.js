...
const resolveMember = async ({message, search, useMessageContent = true}) => {
  ...
  if (contentToCheck.match(/^<@!?(\d+)>$/)) {
    ...
    const memberFound =
        await message.guild.members.fetch(userID).catch(() => {});
    if (memberFound)
      return memberFound;
  }
  // Try with ID
  ...
  if (message.guild.members.cache.some(member =>
                                           member.user.tag === search ||
                                           member.user.username === search)) {
    const memberFound = message.guild.members.cache.find(
        member =>
            member.user.tag === search || member.user.username === search);
    if (memberFound)
      return memberFound;
  }
  return;
...
const resolveRole = async ({message, search}) => {
  ...
  // Try with name with @
  if (message.guild.roles.cache.some(role => `@${role.name}` === search ||
                                             role.name === search)) {
