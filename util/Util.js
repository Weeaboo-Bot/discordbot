...
module.exports = class Util {
  ...
  static async reactIfAble(msg, user, emoji, fallbackEmoji) {
    ...
    if (dm || msg.channel.permissionsFor(user).has(
  ...
  static async pickWhenMany(msg, arr, defaultValue, arrListFunction,
                            {time = 30000} = {}) {
    const resList = arr.map(arrListFunction);
    await msg.reply(stripIdents`
		__**Found ${
        arr.length} results, which would you like to view?**__
		${resList.join('\n')}
		`);
    const filter = (res) => {
  ...
  static async awaitPlayers(msg, max, min = 1) {
    ...
    await msg.say(`You will need at least ${min - 1} more player${
        addS} (at max ${max - 1}). To join, type \`join game\`.`);
    const joined = [];
