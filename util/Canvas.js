...
module.exports = class CanvasUtil {
  ...
  static desaturate(ctx, level, x, y, width, height) {
    ...
    for (let i = 0; i < height; i++) {
      for (let j = 0; j < width; j++) {
        ...
        const grey = Number.parseInt(0.2125 * data.data[dest] +
  ...
  static distort(ctx, amplitude, x, y, width, height, strideLevel = 4) {
    ...
    for (let i = 0; i < width; i++) {
      for (let j = 0; j < height; j++) {
        const xs =
            Math.round(amplitude * Math.sin(2 * Math.PI * 3 * (j / height)));
        const ys =
            Math.round(amplitude * Math.cos(2 * Math.PI * 3 * (i / width)));
  ...
  static fishEye(ctx, level, x, y, width, height) {
    ...
    for (let i = 0; i < frame.data.length; i += 4) {
      ...
      const x2 = Math.round(frame.width / 2 -
                            dx * Math.sin(dist / (level * Math.PI) / 2));
      const y2 = Math.round(frame.height / 2 -
                            dy * Math.sin(dist / (level * Math.PI) / 2));
  ...
  static pixelize(ctx, canvas, image, level, x, y, width, height) {
    ...
    ctx.drawImage(canvas, x, y, width * level, height * level, x, y, width,
                  height);
