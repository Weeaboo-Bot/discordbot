	...
	initCards() {
		for (let i = 0; i < this.suits.length; i++) {
			for (let x = 0; x < this.card_values.length; x++) {
				...
				this.deck[cardNumber] = new Card(this.card_values[x], this.suits[i], x + 1, './assets/images/deck/' + this.card_values[x] + this.suits[i] + '.png', this.uits_names[i], this.card_values[x] + this.suits[i]);
				if(x > 9) {
					this.deck[cardNumber].gameVal = 10;
				}
				if(x === 0) {
					this.deck[cardNumber].gameVal = 11;
				}
			}
		}
	}
