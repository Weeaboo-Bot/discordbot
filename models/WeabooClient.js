...
const {Collection, MessageEmbed, Structures} = require('discord.js');
...
module.exports = class Client extends CommandoClient {
  constructor(options) {
    ...
    this.databaseCache.usersReminds =
        new Collection(); // members with active reminds
    this.databaseCache.mutedUsers =
        new Collection(); // members who are currently muted
  }

  onError(err, message, args, fromPattern,
  ...
  translate(key, args, locale) {
    ...
    if (!language)
      throw 'Invalid language set in data.';
  ...
  printDate(date, format, locale) {
    ...
    if (!format)
      format = languageData.defaultMomentFormat;
  ...
  async findOrCreateUser({id : userID}, isLean) {
    if (this.databaseCache.users.get(userID)) {
      return isLean ? this.databaseCache.users.get(userID).toJSON()
                    : this.databaseCache.users.get(userID);
    } else {
      let userData =
          (isLean ? await this.usersData.findOne({id : userID}).lean()
  ...
  async findOrCreateMember({id : memberID, guildID}, isLean) {
    if (this.databaseCache.members.get(`${memberID}${guildID}`)) {
      return isLean ? this.databaseCache.members.get(`${memberID}${guildID}`)
                          .toJSON()
  ...
  async findOrCreateGuild({id : guildID}, isLean) {
    if (this.databaseCache.guilds.get(guildID)) {
      return isLean ? this.databaseCache.guilds.get(guildID).toJSON()
                    : this.databaseCache.guilds.get(guildID);
    } else {
      let guildData = (isLean ? await this.guildsData.findOne({id : guildID})
                                    .populate('members')
                                    .lean()
