const mongoose = require("mongoose"), Canvas = require("canvas");
...
const userSchema = new mongoose.Schema({
  ...
  /* STATS */
  registeredAt :
      {type : Number, default : Date.now()}, // Registered date of the user
  ...
  logged : {
    type : Boolean,
    default : false
  }, // if the user is logged to the dashboard
...
userSchema.method("getAchievements", async function() {
  const canvas = Canvas.createCanvas(1800, 250), ctx = canvas.getContext("2d");
  const images = [
    await Canvas.loadImage("./assets/img/achievements/achievement" +
                           (this.achievements.work.achieved ? "_colored" : "") +
                           "1.png"),
    await Canvas.loadImage(
        "./assets/img/achievements/achievement" +
        (this.achievements.firstCommand.achieved ? "_colored" : "") + "2.png"),
    await Canvas.loadImage(
        "./assets/img/achievements/achievement" +
        (this.achievements.married.achieved ? "_colored" : "") + "3.png"),
    await Canvas.loadImage(
        "./assets/img/achievements/achievement" +
        (this.achievements.slots.achieved ? "_colored" : "") + "4.png"),
    await Canvas.loadImage("./assets/img/achievements/achievement" +
    ...
    await Canvas.loadImage(
        "./assets/img/achievements/achievement" +
        (this.achievements.invite.achieved ? "_colored" : "") + "7.png")
  ];
  let dim = 0;
