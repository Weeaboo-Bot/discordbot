...
module.exports = {
  run : (oldGuild, newGuild) => {
    if (!client.provider.isReady)
      return;
    if (!client.provider.getGuild(newGuild.id, 'prefix'))
      return;

    const lang = require(`../languages/${
        client.provider.getGuild(newGuild.id, 'language')}.json`);
    ...
    if (oldGuild.iconURL() !== newGuild.iconURL()) {
      const embed = new Discord.MessageEmbed()
                        ...
                        .addField(`📤 ${lang.guildupdateevent_oldservericon}`,
                                  oldGuild.iconURL() === null
                                      ? lang.guildupdateevent_noservericon
                                      : oldGuild.iconURL())
                        .addField(`📥 ${lang.guildupdateevent_newservericon}`,
    ...
    if (oldGuild.owner.id !== newGuild.owner.id) {
      const embed = new Discord.MessageEmbed()
                        ...
                        .addField(`📤 ${lang.guildupdateevent_oldowner}`,
                                  oldGuild.owner.user.tag)
                        .addField(`📥 ${lang.guildupdateevent_newowner}`,
