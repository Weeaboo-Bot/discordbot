...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.content.match(new RegExp(`^<@!?${client.user.id}>( |)$`))) {
      if (message.guild) {
        return message.sendT(
            "misc:HELLO_SERVER",
            {username : message.author.username, prefix : data.guild.prefix});
      } else {
    ...
    if (message.guild) {
        ...
        if (channelSlowmode) {
          ...
          if (uSlowmode) {
            if (uSlowmode.time > Date.now()) {
              ...
              return message.author.send(message.translate(
                  "administration/slowmode:PLEASE_WAIT",
                  {time : delay, channel : message.channel.toString()}));
            } else {
      ...
      if (data.guild.plugins.automod.enabled &&
          !data.guild.plugins.automod.ignored.includes(message.channel.id)) {
        if (/(discord\.(gg|io|me|li)\/.+|discordapp\.com\/invite\/.+)/i.test(
                message.content)) {
          if (!message.channel.permissionsFor(message.member)
                   .has("MANAGE_MESSAGES")) {
            message.delete();
      ...
      message.mentions.users.forEach(async (u) => {
        ...
        if (userData.afk) {
          message.error("general/setafk:IS_AFK",
                        {username : u.tag, reason : userData.afk});
        }
      });
    }
    ...
    const args =
        message.content.slice((typeof prefix === "string" ? prefix.length : 0))
            .trim()
            .split(/ +/g);
    ...
    if (message.guild) {
      ...
      if (neededPermissions.length > 0) {
        return message.error(
            "misc:MISSING_MEMBER_PERMS",
            {list : neededPermissions.map((p) => `\`${p}\``).join(", ")});
      }
      if (data.guild.ignoredChannels.includes(message.channel.id) &&
          !message.member.hasPermission("MANAGE_MESSAGES")) {
        message.delete();
        message.author.send(message.translate(
            "misc:RESTRICTED_CHANNEL", {channel : message.channel.toString()}));
        return;
      }
    ...
    client.logger.log(`${message.author.username} (${
...
async function updateXp(msg, data) {
  ...
  // Records in the database the time when the member will be able to win xp
  // again (3min)
  const toWait = Date.now() + 60000;
  xpCooldown[msg.author.id] = toWait;

  // Gets a random number between 10 and 5
  const won = Math.floor(Math.random() * (Math.floor(10) - Math.ceil(5))) +
              Math.ceil(5);
