...
module.exports = {
  run: (role) => {
    ...
    for (
      let i = 0;
      i < client.provider.getGuild(role.guild.id, "selfassignableroles").length;
      i += 1
    ) {
      if (
        role.id ===
        ...
        const currentSelfassignableroles = client.provider.getGuild(
          role.guild.id,
          "selfassignableroles"
        );
        currentSelfassignableroles.splice(i, 1);
        return client.provider.setGuild(
          role.guild.id,
          "selfassignableroles",
