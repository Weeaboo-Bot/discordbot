...
module.exports = async (client, message) => {
  ...
  await message.guild
      ...
      .then((res) => { deletionLog = res.entries.first(); });

  // Let's perform a sanity check here and make sure we got *something*
  if (!deletionLog) {
    return client.logger.info(`A message by ${
        message.author
            .tag} was deleted, but no relevant audit logs were found.`);
  }
  ...
  if (target.id === message.author.id) {
    ...
    const embed = new MessageEmbed()
                      ...
                      .setTimestamp();
    channel.send({embed});
    client.logger.info(`A message by ${
        message.author.tag} was deleted, but we don't know by who.`);
  }
};
