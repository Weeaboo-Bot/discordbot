...
module.exports = {
  run : channel => {
    ...
    if (!client.provider.getGuild(channel.guild.id, 'prefix'))
      return;
    if (client.provider.getGuild(channel.guild.id, 'channeldeletelog') ===
        'false')
      return;
    ...
    const messagechannel = client.channels.get(
        client.provider.getGuild(channel.guild.id, 'channeldeletelogchannel'));
    if (!messagechannel)
      return;
