...
module.exports = class {
  ...
  async run(messages) {
    if (!this.client.provider.isReady)
      return;
    if (messages.size === 0)
      return;
    ...
    messages.forEach(msg => {
      ...
      if (!client.provider.getGuild(msg.guild.id, 'prefix'))
        return;
      ...
      const lang = require(`../languages/${
          client.provider.getGuild(msg.guild.id, 'language')}.json`);

      const embed = new Discord.MessageEmbed()
                        .setColor('RED')
                        .setTimestamp()
                        .setAuthor(lang.messagedeleteevent_deleted)
                        .addField(`🗣 ${lang.messagedeleteevent_author}`,
                                  msg.author.tag)
                        .addField(`📲 ${lang.messagedeleteevent_channel}`,
                                  `${msg.channel.name} (${msg.channel.id})`)
                        .addField(`📎 ${lang.messagedeleteevent_mid}`, msg.id)
                        .addField(`📜 ${lang.messagedeleteevent_message}`,
